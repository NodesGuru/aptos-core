...
pub const NAME: &str = "ng_processor";
pub struct NGTransactionProcessor {
    connection_pool: PgDbPool,
}

impl NGTransactionProcessor {
    pub fn new(connection_pool: PgDbPool) -> Self {
        Self { connection_pool }
    }
}

impl Debug for NGTransactionProcessor {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let state = &self.connection_pool.state();
        write!(
            f,
            "NGTransactionProcessor {{ connections: {:?}  idle_connections: {:?} }}",
...
#[async_trait]
impl TransactionProcessor for NGTransactionProcessor {
    ...
    async fn process_transactions(
        ...
        let (/* txns, */ _, txn_details, events, /* write_set_changes, */ _, wsc_details) =
